...
# 保存数据集
torch.save({
    'obs_traj': dset_train.obs_traj,
    'pred_traj': dset_train.pred_traj,
    'obs_traj_rel': dset_train.obs_traj_rel,
    'pred_traj_rel': dset_train.pred_traj_rel,
    'loss_mask': dset_train.loss_mask,
    'non_linear_ped': dset_train.non_linear_ped,
    'seq_start_end': dset_train.seq_start_end,
    'v_obs': [v_.cpu() for v_ in dset_train.v_obs],  # 确保数据在 CPU 上
    'A_obs': [a_.cpu() for a_ in dset_train.A_obs],
    'v_pred': [v_.cpu() for v_ in dset_train.v_pred],
    'A_pred': [a_.cpu() for a_ in dset_train.A_pred],
    'norm_lap_matr': dset_train.norm_lap_matr,
    'obs_seq_len': dset_train.obs_len,
    'pred_seq_len': dset_train.pred_len
}, './deal_dataset/train_1_dataset_1030.pt')
...
torch.save({
    'obs_traj': dset_val.obs_traj,
    'pred_traj': dset_val.pred_traj,
    'obs_traj_rel': dset_val.obs_traj_rel,
    'pred_traj_rel': dset_val.pred_traj_rel,
    'loss_mask': dset_val.loss_mask,
    'non_linear_ped': dset_val.non_linear_ped,
    'seq_start_end': dset_val.seq_start_end,
    'v_obs': [v_.cpu() for v_ in dset_val.v_obs],  # 确保数据在 CPU 上
    'A_obs': [a_.cpu() for a_ in dset_val.A_obs],
    'v_pred': [v_.cpu() for v_ in dset_val.v_pred],
    'A_pred': [a_.cpu() for a_ in dset_val.A_pred],
    'norm_lap_matr': dset_val.norm_lap_matr,
    'obs_seq_len': dset_val.obs_len,
    'pred_seq_len': dset_val.pred_len
}, './deal_dataset/val_1_dataset_1030.pt')
